...
void Menu::draw() {
   ...
   // Add the game title "BYTERAIDER"
    const std::vector<std::string> gameTitle = {
    ...
    int titleRow = 3; // Place title at row 1 (adjusted from row 2 for better alignment)

    for (const auto& charTexture : gameTitle) {
        menuGridState[titleRow][titleStartCol] = Tile(1.0f, 1.0f, 1.0f); // Set white text
